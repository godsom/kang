...
**Update (post-Milestone 7):** `match_history.pot_amount` is no longer always `0`. Once the in-memory point ledger (`src/server/ledger.js`) and its baht conversion (`GAME_CONFIG.BAHT_PER_POINT`) were added for the live payout matrix, `stats.js`'s `calcPotAmount` derives each player's real net baht for the round (winner: `+multiplier × 5 × loserCount`; loser: `-multiplier × 5 × winnerCount`, matching exactly what the ledger credits) and persists it per-player in `match_history`. This is still not wired to the Auth/Wallet service's real ledger (Milestones 3/4's deferral still stands — no actual money moves), and the leaderboard still ranks by win count (`leaderboard:wins`) rather than profit, but `pot_amount` itself is now a real, non-zero, permanently persisted number.
